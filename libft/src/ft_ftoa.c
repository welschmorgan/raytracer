...
/*   Updated: 2014/02/14 22:02:19 by mwelsch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_float					create_float(double val, int precision)
{
	t_float				r;
...
void					extract_whole(t_float *f, int dig_len, char **ptr)
{
	long				i;
	long				tmp;
...
void					extract_decimal(t_float *f, char **ptr)
{
	long				i;
	double				d;
...
char					*ft_ftoa(double n, int precision)
{
	...
	extract_whole(&f, i + 1, &bufptr);
	*bufptr++ = '.';
	extract_decimal(&f, &bufptr);
	*bufptr = '\0';
	return (buf);
}
